...
- With kvm + minikube and helm 3, there was a timing issue: `helm upgrade` sometimes failed due to CRDs unavailable - s. [fix: clear the discovery cache after CRDs are installed](https://github.com/helm/helm/pull/6332). Helm only waited for CRDs to *exist*, not to be `Established` in API discovery, and a cold kvm2 apiserver lost that race. It does not reproduce under the pinned helm 4 / helmfile 1 toolchain, so the former CRD pre-apply + `sleep 60` workaround has been removed and `apply-apps` is a single `helmfile sync`.
